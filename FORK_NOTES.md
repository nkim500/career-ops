...
|------|-----------|---------|-------|
| 2026-05-14 | `TBD` | Remove PDF generation from the batch worker prompt | `batch/batch-prompt.md` no longer generates a tailored CV PDF — it produces eval report + tracker line + JSON only. Content folded in from the fork-local `batch/batch-prompt-eval-only.md`, now deleted (it was an exact duplicate once PDF was stripped). PDF wording also cleared from `modes/batch.md`, `batch/batch-runner.sh`, and `batch/README.md`. Ad-hoc PDF generation stays in `auto-pipeline` (single-URL drop). Spec: `docs/superpowers/specs/2026-05-14-batch-pdf-removal-handoff.md`. Plan: `docs/superpowers/plans/2026-05-14-batch-pdf-removal.md`. |
...
| 2026-04-13 | `213eeef` (PR #5) | Batch Sonnet workers + post-worker score fallback | `batch/batch-runner.sh` forces `--model sonnet`, adds `--prompt-file` flag, translates worker prompt to English. New `batch/post-worker.mjs` parses the report the worker wrote and synthesizes `batch/tracker-additions/{id}.tsv` when the worker skipped it. New `batch/batch-prompt-eval-only.md` prompt variant. **Update 2026-05-14:** folded back into `batch/batch-prompt.md` and deleted once PDF generation was removed from the batch worker — see the 2026-05-14 row above. |
